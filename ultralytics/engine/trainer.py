...
class BaseTrainer:
    ...
    def _setup_afss(self, batch_size: int) -> None:
        """Rebuild the training dataloader with AFSS sampling when enabled."""
        self.afss_manager = None
        if not getattr(self.args, "afss", False):
            return
        if self.world_size > 1 or RANK != -1:
            if RANK in {-1, 0}:
                LOGGER.warning("AFSS is currently enabled only for single-process training; disabling AFSS for DDP.")
            return

        from ultralytics.data.build import build_dataloader as _build_dataloader
        from ultralytics.utils.afss import AFSSBatchSampler, AFSSManager, suggest_afss_params

        n_train_images = len(self.train_loader.dataset)
        if getattr(self.args, "afss_auto_tune", False):
            nc = self.data.get("nc", getattr(unwrap_model(self.model), "nc", 10))
            ...
            if not tuned.get("afss", True):
                LOGGER.warning("AFSS auto-tune disabled AFSS for this run.")
                return
        ...
        self.afss_manager = AFSSManager(
            ...
            num_classes=self.data.get("nc", 0),
            class_aware=getattr(self.args, "afss_class_aware", True),
        )
        ...
        LOGGER.info(
            ...
            f"class_aware={self.afss_manager.class_aware}"
        )
            ...
            if RANK in {-1, 0}:
                ...
                if self.afss_manager and hasattr(self, "compute_per_image_metrics"):
                    if self.afss_manager.should_update(epoch + 1):
                        ...
                        afss_metrics = self.compute_per_image_metrics()
                        if len(afss_metrics) == 3:
                            indices, precisions, recalls = afss_metrics
                            self.afss_manager.update_metrics(indices, precisions, recalls, epoch)
                        else:
                            indices, precisions, recalls, class_precisions, class_recalls, class_present = afss_metrics
                            self.afss_manager.update_metrics(
                                indices,
                                precisions,
                                recalls,
                                epoch,
                        ...
                        LOGGER.info(
                            ...
                            f"ClassAware={stats['class_aware']}"
                        )
                        for class_stats in self.afss_manager.get_class_stats(self.data.get("names")):
                            LOGGER.info(
                                f"AFSS class {class_stats['class_id']}({class_stats['name']}): "
                                f"Easy={class_stats['easy']} Moderate={class_stats['moderate']} "
                                f"Hard={class_stats['hard']} Total={class_stats['total']} "
                                f"MeanS={class_stats['mean_sufficiency']:.3f}"
                            )
                        self.afss_manager.record_history(epoch + 1, stats)
                        self.afss_manager.save_history_csv(self.save_dir / "afss_difficulty_history.csv")
                        if self.args.plots:
                            afss_plot = self.save_dir / "afss_difficulty_history.png"
                            self.afss_manager.plot_history(afss_plot)
                            LOGGER.info(f"AFSS: Difficulty history plot saved to {afss_plot}")
                        ...
                        if adapt_info:
                            LOGGER.info(
                                ...
                                f"  easy_ratio: {adapt_info['old_easy_ratio']:.3f} -> "
                                f"{adapt_info['new_easy_ratio']:.3f}\n"
                                f"  moderate_ratio: {adapt_info['old_moderate_ratio']:.3f} -> "
                                f"{adapt_info['new_moderate_ratio']:.3f}\n"
