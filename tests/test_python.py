...
def test_aliyun_api_yolo_helpers(tmp_path):
    """Test helper functions for LLM alert integration without requiring cv2 or dashscope."""
    import aliyun_api_yolo as llm_alert

    source = tmp_path / "source.jpg"
    Image.new("RGB", (200, 100), color="white").save(source)

    detections = llm_alert.normalize_detections(
        [{"bbox": [10, 20, 110, 80], "label": "road-grain", "score": 0.88}],
        image_width=200,
        image_height=100,
    )
    assert detections[0]["xyxy_px"] == [10.0, 20.0, 110.0, 80.0]
    assert detections[0]["xyxy_norm"] == [0.05, 0.2, 0.55, 0.8]

    boxed = llm_alert.draw_boxes(str(source), detections, tmp_path / "boxed.jpg")
    assert Path(boxed).is_file()

    prompt = llm_alert.build_prompt(detections, image_width=200, image_height=100, includes_boxed_image=True)
    assert "占道晒粮" in prompt
    assert '"box_id": 0' in prompt

    parsed = llm_alert.parse_llm_json(
        '```json\n{"items":[{"box_id":0,"should_alert":false}],"image_alert":false,"summary":"ok"}\n```'
    )
    assert parsed["image_alert"] is False
    assert parsed["items"][0]["box_id"] == 0
